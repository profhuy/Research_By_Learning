...
def main():
    ...
    # Match by (repo, issue_id) to avoid ID collision across repos
    llm_by_key = {(r["repo"], r["issue_id"]): r for r in llm_rows}
    ...
    for c in COMPONENTS:
        k = hh_kappa[c]
        if k is not None:
            print(f"  {c}: {k:.3f} ({interpret(k)}) -> {decide(k)}")
        else:
            print(f"  {c}: N/A")
    ...
    with open(summary_path, "w", newline="", encoding="utf-8") as f:
        ...
        for rq, metric, value, p, eff, nv, ni, dec in summary_rows:
            val_str = f"{value:.4f}" if isinstance(value, float) else (value if value is not None else "")
            w.writerow([rq, metric, val_str, p, eff, nv, ni, dec])

    print(f"\nWrote {summary_path}")

    # --- Ghi pilot_analysis_plan.md CHỈ khi mode pilot ---
    if args.mode == "pilot":
